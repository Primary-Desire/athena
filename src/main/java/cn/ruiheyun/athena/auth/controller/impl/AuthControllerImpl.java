...
public class AuthControllerImpl implements IAuthController {
    ...
    public Object signUp(@RequestBody JSONObject requestBody) {
        return Mono.just(requestBody).map(request -> request.toJavaObject(SysUser.class))
                ...
                .doOnNext(sysUser -> reactiveStringRedisUtils.get(RSAUtils.PRIVATE_KEY)
                        .doOnNext(privateKeyStr -> {
                            PrivateKey privateKey = RSAUtils.base64ToPrivateKey(privateKeyStr);
